...
- `venues` 테이블의 `sections`를 비정규화하여 저장 (성능 최적화)
...
- 장애 복구(DR) 시 DR_RECOVERED 상태로 15분 유예기간 부여
...
| **DR_RECOVERED** | `ttl` | **15분** | 장애 복구 유예기간, 만료 시 자동 삭제 |
| **CANCELLED** | `ttl` | **7일** | 취소 후 7일간 보존 (사용자 이력), 이후 자동 삭제 |
| **CONFIRMED** | - | 없음 | 영구 보존 (취소 전까지) |
...
│       ▼                                                         │
│  ┌──────────────┐                                               │
│  │ DR_RECOVERED │  ← ttl = 현재시간 + 15분 (Grace Period)        │
│  └──────┬───────┘                                               │
│         │                                                       │
│         ├───── 15분 경과 ────→ [자동 삭제]                        │
│         │                                                       │
│         │ 사용자 결제 완료 (confirm_reservation)                 │
│         ▼                                                       │
│  ┌───────────┐                                                  │
│  │ CONFIRMED │                                                  │
│  └───────────┘                                                  │
...
| `totalSeats` | N | `1240` | 총 좌석 수 |
| `availableSeats` | N | `1240` | 잔여 좌석 수 |
...
- 데이터 초기화: performances 테이블에 `sections` 비정규화 복사
...
│     title         │         │     name          │
│   venueId ────────┼────────▶│   address         │
│  seatGrades(SSOT) │         │   sections        │◀── 비정규화 복사
│     cast          │         │   totalSeats      │
│  hasOPSeats       │         └───────────────────┘
│  sections(복사)   │
└────────┬──────────┘
...
| DR_RECOVERED | `ttl` | Unix timestamp (초) | **15분** | 장애 복구 유예기간 |
| CANCELLED | `ttl` | Unix timestamp (초) | **7일** | 취소 이력 보존 후 삭제 |
...
```typescript
// performanceId로 필터링 (GSI 없어서 Scan)
const result = await dynamoDb.send(new ScanCommand({
...
| 상태 | 설명 | 챗봇 표시 | TTL |
|-----|------|----------|-----|
| `HOLDING` | 좌석 선점 중 | "선점 중 - 60초 내 결제 필요" | 60초 |
| `CONFIRMED` | 예약 확정됨 | "✅ 예약 완료" | 없음 |
| `CANCELLED` | 취소됨 (Soft Delete) | 조회 결과에서 제외 | 7일 |
| `DR_RECOVERED` | 장애 복구 후 선점 복원됨 | "⚠️ 복구됨 - 결제 진행 필요" | 15분 |

### DR_RECOVERED가 필요한 이유

```
정상 흐름:
  HOLDING (60초) → 결제 → CONFIRMED

장애 발생 시:
  HOLDING → Main 리전 장애! → DR 전환
                    ↓
          60초 타이머 만료됨 (정상이면 취소)
          하지만 장애 상황이므로 사용자 보호 필요
                    ↓
          HOLDING → DR_RECOVERED (15분 Grace Period)
          "다시 결제할 기회를 드립니다"
```

### CONFIRMED vs DR_RECOVERED 차이

| 상태 | 의미 | 결제 완료 | 사용자 액션 |
|-----|------|----------|------------|
| CONFIRMED | 정상 예약 완료 | ✅ 완료 | 없음 (예약 확인만) |
| DR_RECOVERED | 장애 복구 후 선점 복원 | ❌ 대기 | [결제하기] 또는 [취소하기] |
...
| `schedules` | GSI 이미 존재 (`performanceId-datetime-index`) | ❌ 불필요 |
...
| **performances** | `sections` | venues.sections | 공연 등록 시 | 좌석 배치도 조회 최적화 |
...
| `KDT-Msp4-PLDR-schedules` | ✅ 자동 복제 | `performanceId-datetime-index` 자동 포함 |
...
| `performanceId-datetime-index` GSI (schedules) | ✅ 이미 사용 중 | - |
