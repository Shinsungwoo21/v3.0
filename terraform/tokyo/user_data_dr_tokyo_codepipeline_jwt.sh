#!/bin/bash
# =============================================================================
# DR Tokyo User Data - CodePipeline용 (S3 아티팩트 다운로드 방식)
# =============================================================================
...
#       - PM2로 앱 재시작
# 
# GitHub Actions와 분리: codepipeline-latest/app.zip 경로 사용
# =============================================================================

set -x
exec > >(tee /var/log/user-data.log) 2>&1
echo "=== DR Tokyo (CodePipeline) User Data Started: $(date) ==="

# -----------------------------------------------------------------------------
# 변수 설정 (CodePipeline용 경로)
# -----------------------------------------------------------------------------
APP_DIR=/home/ec2-user/app
S3_BUCKET=plcr-s3-an1-app-artifacts
S3_KEY=codepipeline-latest/app.zip    # GitHub Actions(latest/)와 분리
...
export PIPELINE_TYPE=codepipeline
ENVEOF
...
# -----------------------------------------------------------------------------
# 4. S3에서 최신 아티팩트 다운로드 (CodePipeline 전용 경로)
# -----------------------------------------------------------------------------
...
echo "=== DR Tokyo (CodePipeline) User Data Completed: $(date) ==="